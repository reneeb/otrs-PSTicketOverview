...
# --
# $Id: Ticket.pm,v 1.16 2011/07/24 09:16:12 ud Exp $
# $OldId: Ticket.pm,v 1.486 2011/01/05 09:42:54 bes Exp $
# --
...
# ---
    ...
    if ( $Param{Fulltext} && $Param{Fulltext} =~ m{ \A [#*]? \d+ [*]? \z }xms ) {
        my $Tn = $Param{Fulltext};
        $Tn    =~ s{ \A [#] }{}xms;

        $Param{TicketNumber} = [ '%' . $Tn . '%' ];
    }
...
# DSV
# ---
    my %SortByArticleColumns = (
        From    => 'art.a_from',
        Subject => 'art.a_subject',
    );

    my %SortByDSVColumns = (
        Anwendung => 'dt.ticket_anwendung',
    );
# ---
...
#        if ( !$SortOptions{ $SortByArray[$Count] } ) {
        $SortByArticleColumn = 1 if $SortByArticleColumns{ $SortByArray[$Count] };
        $SortByDSVColumn     = 1 if $SortByDSVColumns{ $SortByArray[$Count] };

        if (
            !$SortOptions{ $SortByArray[$Count] } &&
            !$SortByArticleColumns{ $SortByArray[$Count] } &&
            !$SortByDSVColumns{ $SortByArray[$Count] }
            )
        {
# ---
            $Self->{LogObject}->Log(
...
#            $SQL .= ', ' . $SortOptions{$SortBy};
            $SQL .= ', ' . (
                $SortOptions{$SortBy} ||
                $SortByArticleColumns{$SortBy} ||
                $SortByDSVColumns{$SortBy}
            );
# ---
        }
    }
...
#    $SQL .= ' FROM ticket st, queue sq ';
    $SQL .= ' FROM ticket st INNER JOIN queue sq ON st.queue_id = sq.id ';
# ---
...
#    my $SQLExt = $ArticleSQLExt;
    my $SQLExt = '';
    $SQL .= 'INNER JOIN article art ON art.ticket_id = st.id '  if $ArticleSQL;
    $SQL .= 'INNER JOIN dsv_ticket dt ON dt.ticket_id = st.id ' if $SortByDSVColumn;
...
# DSV
# ---
        last if defined $Param{Fulltext};
# ---
...
# ---
    ...
    if ( $Param{Fulltext} ) {
        $ArticleIndexSQLExt =~ s{ \A \s* AND }{OR}xms;
        my $Tn = $Param{TicketNumber}->[0] || '%';
        $ArticleIndexSQLExt = "AND ( st.tn LIKE '$Tn' $ArticleIndexSQLExt )";
    }
...
# ---
    # get tickets changed older than x minutes
    if ( defined $Param{DSVTicketChangeTimeOlderMinutes} ) {

        $Param{DSVTicketChangeTimeOlderMinutes} ||= 0;

        my $TimeStamp = $Self->{TimeObject}->SystemTime();
        $TimeStamp -= ( $Param{DSVTicketChangeTimeOlderMinutes} * 60 );

        $Param{DSVTicketChangeTimeOlderDate} = $Self->{TimeObject}->SystemTime2TimeStamp(
            SystemTime => $TimeStamp,
        );
    }
# ---
...
# ---
    # get tickets changed older than xxxx-xx-xx xx:xx date
    if ( $Param{DSVTicketChangeTimeOlderDate} ) {

        # check time format
        if (
            $Param{DSVTicketChangeTimeOlderDate}
            !~ /\d\d\d\d-(\d\d|\d)-(\d\d|\d) (\d\d|\d):(\d\d|\d):(\d\d|\d)/
            )
        {
            $Self->{LogObject}->Log(
                Priority => 'error',
                Message  => "Invalid time format '$Param{DSVTicketChangeTimeOlderDate}'!",
            );
            return;
        }

        $SQLExt .= " AND st.change_time <= '"
            . $Self->{DBObject}->Quote( $Param{DSVTicketChangeTimeOlderDate} ) . "'";
    }
# ---
...
#            $SQLExt .= ' ' . $SortOptions{ $SortByArray[$Count] };
            my $Key = $SortByArray[$Count];
            $SQLExt .= ' ' . (
                $SortOptions{$Key} ||
                $SortByArticleColumns{$Key} ||
                $SortByDSVColumns{$Key}
            );
# ---
...
# ---
#        if ($CacheData) {
        if ( $CacheData && !$Param{DSVResolvedTickets} ) {
...
# DSV
# ---
    if ( $Param{DSVResolvedTickets} && @TicketIDs) {

        %Tickets = ();

        my $Slot       = $Self->{ConfigObject}->Get( 'DSV::ResolvedToClosedSeconds' ) || (60 * 60 * 24 * 2);
        my $SystemTime = $Self->{TimeObject}->SystemTime();
        my $Time       = $Self->{TimeObject}->SystemTime2TimeStamp(
            SystemTime => ( $SystemTime - $Slot ),
        );

        my $TicketIDString = join ', ', @TicketIDs;
        my $SQL = 'SELECT ticket_history.name, ticket_id, ticket_history.create_time FROM ticket_history '
            . 'INNER JOIN ticket_history_type ON history_type_id = ticket_history_type.id '
            . "WHERE ticket_id IN ($TicketIDString) AND ticket_history_type.name = 'StateUpdate' "
            . "AND ticket_history.create_time <= '$Time' ORDER BY ticket_history.id DESC";

        return if !$Self->{DBObject}->Prepare( SQL => $SQL, Limit => 1 );
...
$Revision: 1.16 $ $Date: 2011/07/24 09:16:12 $
